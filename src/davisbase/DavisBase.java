...
import command.InsertIntoCommand;
...
public class DavisBase {
    ...
    public static void parseUserCommand(String userCommand) {
        ...
        switch (commandTokens[0]) {
            ...
            case "update":
                ...
                break;
            case "insert":
                InsertIntoCommand insertCmd = parseInsertIntoTableCmd(userCommand);
                if(insertCmd !=null){
                    CommandExecuter.getSingle().insertIntoValues(insertCmd.tableName, insertCmd.valuesToInsert);
                }
            ...
            case "create":
                ...
                break;
            case "drop":
                String tabName = parseDropTableCmd(userCommand);
                if (tabName == null) {
                    System.out.println("Invalid command");
                    break;
                }
                CommandExecuter.getSingle().dropTable(tabName);
                System.out.println("Table drop called");
                break;
            case "help":
                help();
                break;
            case "version":
                ...
                break;
            case "exit":
                DatabaseInfo.getSingle().saveFilesToSystem();
                break;
            default:
    ...
    static String parseDropTableCmd(String sCmd) {
        String tableName = ParsingUtility.getPatternMatchKey("drop table (\\S*)", sCmd);
        if (ParsingUtility.checkIsEmpty(tableName)) {
            return tableName;
        }
        return null;
    }
    ...
    private static InsertIntoCommand parseInsertIntoTableCmd(String sCmd) {
        InsertIntoCommand cmd = new InsertIntoCommand();
        sCmd = renameKey(sCmd);
        String tblName = ParsingUtility.getPatternMatchKey("insert into (.*?) values\\(", sCmd);
        tblName = tblName.trim();
        if (ParsingUtility.checkIsEmpty(tblName)) {
            cmd.setTableName(tblName);
            String midStr = ParsingUtility.getParanthesisValue(sCmd);
            ...
            for (int i = 0; i < values.length; i++) {
                if (values[i].contains("'")) {
                    String xx = ParsingUtility.getSingleQuoteValue(values[i]);
                    if (xx == null) {
                        System.out.println("values is not in right format");
                        return null;
                    }
                    values[i] = xx;
                }
            ...
            System.out.println("insert into table command is not right");
        }
        return cmd;
    }
